        ...
        >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center h-20">
                    {/* Logo - Left */}
                    <Link href="/" className="flex items-center group shrink-0" aria-label="BN Media Home">
                        <div className="relative">
                            <div className="absolute inset-0 bg-[#0066FF]/20 blur-sm opacity-0 group-hover:opacity-100 transition-opacity rounded" />
                    ...
                    <div className="hidden md:flex items-center justify-center flex-1 space-x-1">
                        {navItems.map((item) => (
                            <Link
                                key={item.href}
                                href={item.href}
                                className={`relative px-4 py-2 text-base font-medium transition-all duration-300 group ${isActive(item.href) ? "text-[#0066FF]" : "text-white hover:text-[#0066FF]"
                                    }`}
                            >
                                {item.label}
                                <span
                                    className={`absolute bottom-0 left-0 w-full h-0.5 bg-[#0066FF] transform transition-transform duration-300 ${isActive(item.href) ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"
                                        }`}
                                />
                            </Link>
                        ))}
                    ...
                    <div className="hidden md:flex items-center shrink-0">
                        <Link
                            href="/contact"
                            className="bg-[#0066FF] text-white px-6 py-2.5 rounded-lg font-semibold hover:bg-[#0052CC] hover:shadow-lg hover:shadow-[#0066FF]/50 transition-all duration-300 hover:scale-105"
                        >
                            Get Started
                        </Link>
                    </div>
            ...
            {/* Glowing Bottom Line */}
            <div className="absolute bottom-0 left-0 w-full h-0.5 bg-[#0066FF] shadow-[0_0_6px_1px_#0066FF]" />
        </nav>
    );
}
